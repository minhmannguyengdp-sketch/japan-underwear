...
function resolveBasePrice(product) {
  ...
  if (prices.length !== 1) {
    throw new Error(
      `Model ${productKey(product)} phải có đúng một giá cơ bản trong bảng giá; nhận được ${prices.length}.`,
    );
  }
  return prices[0];
}
...
function validateSources(manifest, report, allowWarnings) {
  ...
  const declaredProducts = Number(manifest.summary?.productGroupCount ?? -1);
  const declaredImages = Number(manifest.summary?.matchedImageCount ?? -1);
  if (declaredProducts !== manifest.products.length) {
    throw new Error(
      `Số model trong manifest không khớp: summary=${declaredProducts}, products=${manifest.products.length}.`,
    );
  }
  ...
  if (warningCount > 0 && !allowWarnings) {
    throw new Error(
      `Manifest còn ${warningCount} cảnh báo identity. Chỉ dùng --allow-warnings sau khi đã duyệt.`,
    );
  }
  ...
  const retainedNoImageKeys = new Set(
    (manifest.retainedNoImageProducts ?? []).map((item) => item.key),
  );
  for (const key of retainedNoImageKeys) {
    if (!productKeys.has(key)) {
      throw new Error(`Ngoại lệ giữ không ảnh không thuộc manifest active: ${key}.`);
    }
  }

  if (report.mode !== "apply") throw new Error("R2 report không phải apply report.");
  if (!Array.isArray(report.objects)) throw new Error("R2 report không có danh sách objects.");
  if (report.objects.length !== declaredImages) {
    throw new Error(
      `R2 report chưa khớp manifest active: objects=${report.objects.length}, expected=${declaredImages}. Chạy npm run catalog:sync.`,
    );
  }
  ...
  for (const object of report.objects) {
    ...
    if (!productKeys.has(key)) {
      throw new Error(`R2 report có object không thuộc manifest active: ${key}.`);
    }
  ...
  for (const product of manifest.products) {
    ...
    if (images.length === 0 && !retainedNoImageKeys.has(key)) {
      throw new Error(`Model ${key} không có object R2 và không phải ngoại lệ đã duyệt.`);
    }
    if (images.length > 0 && retainedNoImageKeys.has(key)) {
      throw new Error(`Model ngoại lệ ${key} đã có ảnh; cần bỏ khỏi danh sách giữ-không-ảnh.`);
    }
    images.sort(
      (left, right) =>
  ...
  return { objectsByProduct, warningCount, retainedNoImageKeys };
}

function buildPayload(manifest, objectsByProduct, retainedNoImageKeys) {
  const brands = [
    ...new Map(
  ...
  for (const product of manifest.products) {
    const key = productKey(product);
    const productImages = objectsByProduct.get(key) ?? [];
    ...
    const retainedWithoutImage = retainedNoImageKeys.has(key);
    ...
    products.push({
      ...
      description: retainedWithoutImage
      ...
      sourceProductId: `${LOCAL_SOURCE_PREFIX}${key}`,
    });
...
const { objectsByProduct, warningCount, retainedNoImageKeys } = validateSources(
  manifest,
  report,
  options.allowWarnings,
);
const payload = buildPayload(manifest, objectsByProduct, retainedNoImageKeys);
...
console.log(`Model giữ active không ảnh: ${retainedNoImageKeys.size}`);
console.log(`Cảnh báo identity: ${warningCount}`);
...
try {
  ...
  const runResult = await client.query(
    ...
    `,
    [
      "price-authoritative-manifest+r2-report",
      manifestHash,
      JSON.stringify({
        plannedProducts: payload.products.length,
        plannedImages: payload.images.length,
        retainedWithoutImages: retainedNoImageKeys.size,
        warningCount,
      }),
    ],
  );
  ...
  console.log(`[3/5] Đồng bộ ${payload.products.length} model và giá cơ bản...`);
  ...
  console.log(`[4/5] Đồng bộ ${payload.images.length} ảnh theo batch ${IMAGE_BATCH_SIZE}...`);
  for (let offset = 0; offset < payload.images.length; offset += IMAGE_BATCH_SIZE) {
    ...
    console.log(`      ${Math.min(offset + batch.length, payload.images.length)}/${payload.images.length} ảnh`);
  }
  ...
  const verifyResult = await client.query(
    `
      SELECT
        (SELECT COUNT(*)::integer FROM japan_underwear.products
         WHERE id = ANY($1::uuid[]) AND is_active = true) AS products,
        (SELECT COUNT(*)::integer FROM japan_underwear.product_images
         WHERE r2_key = ANY($2::text[])) AS images,
        (SELECT COUNT(*)::integer FROM japan_underwear.products
         WHERE source_product_id LIKE 'local:%' AND is_active = true) AS active_products,
        (SELECT COUNT(*)::integer
         ...
         GROUP BY product.id
         HAVING COUNT(image.id) = 0) AS no_image_products
    `,
    [productIds, importedR2Keys],
  );

  const row = verifyResult.rows[0];
  const verifiedProducts = Number(row.products);
  const verifiedImages = Number(row.images);
  const activeProducts = Number(row.active_products);
  ...
  if (
    ...
    noImageProducts !== retainedNoImageKeys.size
  ) {
    throw new Error(
      `Hậu kiểm không khớp: products=${verifiedProducts}/${payload.products.length}, images=${verifiedImages}/${payload.images.length}, active=${activeProducts}/${payload.products.length}, noImage=${noImageProducts}/${retainedNoImageKeys.size}.`,
    );
  }
  ...
  await client.query(
    ...
    [
      ...
      JSON.stringify({
        ...
        retainedWithoutImages: noImageProducts,
        brands: payload.brands.length,
        categories: payload.categories.length,
        warningCount,
        mode: "price-authoritative-batch",
      }),
    ],
  );
  ...
  console.log(`Products: ${verifiedProducts}. Images: ${verifiedImages}.`);
  console.log(`Active không ảnh đã duyệt: ${noImageProducts}.`);
} catch (error) {
