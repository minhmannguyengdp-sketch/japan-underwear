...
function choosePrimaryProductPrefix(prefixes) {
  if (prefixes.includes("QG")) return "QG";
  if (prefixes.includes("QL")) return "QL";
  if (prefixes.includes("AL")) return "AL";
  return null;
}
...
function inferModelIdentity(relativePath) {
  ...
  for (const segment of segments) {
    const match = segment.match(
      /(?:^|[^a-z0-9])((?:(?:AL|QL|QG)){1,3})[\s_-]*(\d{3,5})(?=$|[^0-9])/i,
    );

    if (!match) continue;

    const productPrefixes = [
      ...new Set(match[1].toUpperCase().match(/AL|QL|QG/g) ?? []),
    ];

    return {
      modelCode: match[2],
      productPrefixes,
      primaryProductPrefix: choosePrimaryProductPrefix(productPrefixes),
      source: "path-product-prefix",
    };
  }
...
function inferBrand(relativePath, brandHint, modelCode) {
  ...
  if (/(^|[^a-z])(pensee|ps)([^a-z]|$)/i.test(normalized)) {
    return { value: "pensee", source: "path-brand" };
  }

  if (/(^|[^a-z])(winking|wk)([^a-z]|$)/i.test(normalized)) {
    return { value: "winking", source: "path-brand" };
  }
  ...
  const modelRule = findModelRule(modelCode);
  if (modelRule) {
    return {
      value: modelRule.brand,
      source: `model-prefix-${modelRule.prefix}`,
    };
  }
...
function inferCategory(relativePath, categoryHint, modelIdentity) {
  const explicitPrefix = modelIdentity.primaryProductPrefix;

  if (explicitPrefix) {
    return {
      value: PRODUCT_PREFIX_RULES[explicitPrefix],
      source: `path-product-prefix-${explicitPrefix}`,
    };
  }
  ...
  const modelRule = findModelRule(modelIdentity.modelCode);
  if (modelRule) {
    return {
      value: modelRule.category,
      source: `model-prefix-${modelRule.prefix}`,
    };
  }
  ...
  const explicitCategories = [
    ...new Set(
      product.productPrefixes
        .map((prefix) => PRODUCT_PREFIX_RULES[prefix])
        .filter(Boolean),
    ),
  ];

  if (product.brand === "unclassified") {
    warnings.push({
      code: "brand-unclassified",
      message: "Không xác định được thương hiệu.",
    });
  }

  if (product.category === "unclassified") {
    warnings.push({
      code: "category-unclassified",
      message: "Không xác định được nhóm sản phẩm.",
    });
  }
...
for (const source of sourceDefinitions) {
  ...
  for (const file of files) {
    ...
    for (const prefix of modelIdentity.productPrefixes) {
      current.productPrefixes.add(prefix);
    }
