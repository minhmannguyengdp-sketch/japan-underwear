...
Lệnh `list` mặc định chỉ hiển thị tối đa 20 đơn `submitted` gần nhất, phù hợp để smoke test transition:

```powershell
cd F:\1_A_Disk_D\TT\japan-underwear

npm run order:status -- list
```

Có thể đổi bộ lọc hoặc giới hạn:
...
Không dùng nguyên chuỗi mẫu `TT-YYYYMMDD-XXXXXXXX`; phải thay bằng một `order_code` được in từ lệnh `list` hoặc mã đơn trả về sau checkout.
